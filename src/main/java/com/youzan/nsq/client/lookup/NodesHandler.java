...
public class NodesHandler implements ResponseHandler<List<NSQNode>> {
    ...
    @Override
    public List<NSQNode> handleResponse(HttpResponse response) throws ClientProtocolException, IOException {

        StatusLine status = response.getStatusLine();
        if (status.getStatusCode() != 200) {
            log.error("handle nsqlookup response fail, http status code {}, cause: {}", status.getStatusCode(),
